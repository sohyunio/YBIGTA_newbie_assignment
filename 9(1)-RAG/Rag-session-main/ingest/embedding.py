...
def embed_passages(texts: list[str], ids: list[str], progress_callback=None) -> np.ndarray:
    ...
    # 2. 배치 처리를 위한 인덱스 생성
    batch_indices = list(range(0, total_texts, BATCH_SIZE))
    ...
    with ThreadPoolExecutor(max_workers=num_clients) as executor:
        ...
        # 4. 결과 취합 및 진행률 표시
        with tqdm(total=total_texts, desc="Embedding Passages") as pbar:
                for future in as_completed(futures):
                    start, end = futures[future]
                    batch_res = future.result()
                    all_embeddings[start:end] = batch_res
                    
                    pbar.update(end - start)
                    if progress_callback:
                        progress_callback(pbar.n, total_texts)
